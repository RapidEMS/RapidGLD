...
import com.github.RapidEMS.sim.stats.TrackerFactory;
import com.github.RapidEMS.utils.Arrayutils;
...
public class SimModel extends Model implements XMLSerializable {
    ...
    protected void moveLane(Drivelane lane) throws InfraException {
        ...
        while (queueIterator.hasNext()) {
            try {
                roadUser = (Roaduser) queueIterator.next();
            }
            ...
            if (!roadUser.didMove(curCycle)) {                                            // Only attempt to move this RU when it hasnt already
                roadUser.setCycleAsked(curCycle);
                node = sign.getNode();
                ru_pos = roadUser.getPosition();
                ru_speed = roadUser.getSpeed();
                ru_len = roadUser.getLength();
                ru_type = roadUser.getType();
                ru_des = roadUser.getDestNode().getId();


                PosMov[] posMovs = calcPosMovs(node, sign, lane, roadUser, queueIterator);

                roadUser.setInQueueForSign(false);
                if (lane.getFreeUnitsInFront(roadUser) < ru_speed)
                    roadUser.setInQueueForSign(true);
                ...
                if (ru_pos - ru_speed < 0) {                                       // Handle Roadusers that possibly can cross a Node
                    if (node instanceof SpecialNode) {                           // Handle Roadusers that get to Special Nodes
                        if (ru_pos == 0 || 0 == lane.getPosFree(queueIterator, 0, ru_len, ru_speed, roadUser)) {
                            node.processStats(roadUser, curCycle, sign);
                            roadUser.setPosition(-1);
                            roadUser.setPrevSign(-1);
                            queueIterator.remove();
                            roadUser.setWaitPos(sign.getId(), sign.mayDrive(), ru_pos);
                            roadUser.setInQueueForSign(false);
                            tlc.updateRoaduserMove(roadUser, lane, sign, ru_pos, null, null, 0, posMovs, null);
                            ((SpecialNode) (node)).enter(roadUser);
                            roadUser = null;
                        }
                    } else if (lane.getSign().mayDrive()) {                        // Handle Roadusers that are (or nearly) at a Sign
                        if (ru_pos == 0 || 0 == lane.getPosFree(queueIterator, 0, ru_len, ru_speed, roadUser)) {  // Can cross-check
                            destLane = dp.getDirection(roadUser, lane, node);
                            if (destLane == null) {
                                destLane = node.getOutboundLanes()[0];
                            }
                            if (destLane.isLastPosFree(ru_len)) {                // Check if there is room on the node
                                try {
                                    node.processStats(roadUser, curCycle, sign);      // Let the RU touch the Sign to refresh/unload some statistical data
                                    destLane.addRoaduserAtEnd(roadUser);
                                    roadUser.setPrevSign(lane.getSign().getId());
                                    queueIterator.remove();                                // Remove the RU from the present lane, and place it on the destination lane
                                    roadUser.setWaitPos(sign.getId(), sign.mayDrive(), ru_pos);
                                    roadUser.setInQueueForSign(false);
                                    tlc.updateRoaduserMove(roadUser, lane, sign, ru_pos, destLane, destLane.getSign(), roadUser.getPosition(), posMovs, destLane);
                                } catch (Exception e) {
                                    System.out.println("Something screwed up in SimModel.moveLane where a Roaduser is about to cross:" + e);
                                }
                            } else {                                            // Otherwise, check if the next lane should move, and then do just that
                                if (curCycle != destLane.getCycleAsked() && curCycle != destLane.getCycleMoved()) {     // If the position is not free, then check if it already moved this turn, if not:
                                ...
                                if (destLane.isLastPosFree(ru_len)) {            // Ok now the lane that should have moved, moved so try again .........
                                    ...
                                    if (moveRoaduserOnLane(queueIterator, roadUser, ru_speed, lane) > 0) {
                                        roadUser.setWaitPos(sign.getId(), sign.mayDrive(), ru_pos);
                                    }
                                    tlc.updateRoaduserMove(roadUser, lane, sign, ru_pos, lane, sign, roadUser.getPosition(), posMovs, null);
                                }
                            }
                        }
                    } else { // Red light. Try to move user as far as it can go on this lane. Update it's move.
                        Drivelane targetLane = lane;
                        ...
                        if (RapidEMS.useRapidEMS) {
                            if (roadUser instanceof Ambulance) {
                                ...
                                if (RapidEMS.moveLeft(leftLane, lane, roadUser)) {
                                    targetLane = leftLane;
                                    targetSign = targetLane.getSign();
                                    roadUser.setPrevSign(lane.getSign().getId());
                                    queueIterator.remove(); // Remove the RU from the present lane, and place it on the destination lane
                                    roadUser.setWaitPos(targetSign.getId(), targetSign.mayDrive(), ru_pos);
                                } else if (moveRoaduserOnLane(queueIterator, roadUser, ru_speed, lane) > 0) {
                                    roadUser.setWaitPos(sign.getId(), sign.mayDrive(), ru_pos);
                                }
                            } else {
                                Drivelane rightLane = RapidEMS.getRightLane(lane);
                                if (RapidEMS.moveRight(rightLane, lane, roadUser)) {
                        ...
                        tlc.updateRoaduserMove(roadUser, lane, sign, ru_pos, targetLane, targetSign, roadUser.getPosition(), posMovs, null);
                    }
                ...
                if (roadUser != null) {
                    roadUser.setCycleMoved(curCycle);
                }
    ...
    public void spawnNewRoadusers() throws InfraException, ClassNotFoundException {
        ...
        for (int i = 0; i < num_edges; i++) {
            ...
            while (list.hasNext()) {
                ...
                if (r instanceof Ambulance && !edge.isStartNode())
                    continue;
            ...
            for (int j = 0; j < num_freqs; j++) {
                ...
                if (freqs[cur_index].freq >= generator.nextFloat()) {
                    ...
                    if (ruType == RoaduserFactory.AMBULANCE) {
                        if (!edge.isStartNode())
                            continue;
    ...
    private EdgeNode findEndNode() {
        EdgeNode[] nodes = infra.getEdgeNodes_();
        for (EdgeNode node : nodes)
            if (node.isEndNode())
                return node;
        System.out.println("PICK A DAMN END NODE");
        return null;
    }
    ...
    private Drivelane findDrivelaneForRU(Roaduser r, SpecialNode e) {
        ...
        Drivelane[] lanes = e.getShortestPaths(dest.getId(), r.getType()).clone();
        ...
        if (savedBySim) {
            ...
            try {
                tlc = factory.getInstanceForLoad
                        (TLCFactory.getNumberByXMLTagName
